# How Shit Works: launch copy

## Publishing order (for SEO)

1. Merge and deploy the blog post. Check https://darshanpania.me/blog/how-shit-works/ is live.
2. In Google Search Console, run URL Inspection on the post and click "Request Indexing".
3. Wait until Search Console says "URL is on Google" (usually 1 to 3 days).
4. Post the LinkedIn teaser (links to the blog).
5. Publish the X Article (x-article.md), then post the X teaser (links to the X Article).
6. Search the title in Google after a week. If the X Article outranks the blog, trim the X Article.

## LinkedIn teaser (links to the blog)

When I was 10, I was scared of our pressure cooker.

It hissed. It whistled. It looked like it could go off at any moment.

Then a school science assignment made me write about how it works. I made a huge chart, and the fear went away.

Last week, Julie Zhuo's post "AI for No Good Reason" brought that chart back to me. So I built it again, but bigger: How Shit Works, a 3D site that shows everyday appliances from the inside.

The full story, including why I had given up on Opus 5 and what changed with Opus 5.5, is on my blog 👇

https://darshanpania.me/blog/how-shit-works/

(Attach: landing.webp or toaster.webp)

## X teaser (quote or link the X Article)

Option A:

I was scared of pressure cookers until a school chart at 10 explained one.

So I built the chart again, in 3D, with Opus 5.5.

The story of How Shit Works 👇

Option B:

@joulee's "AI for No Good Reason" made me build something for no good reason.

A 3D site that shows how a toaster, a ceiling fan and a door lock work from the inside.

How it started 👇
